...
Uses Locust to simulate user load on the PostgreSQL-based event management system.
Follows project guidelines for proper error handling and PostgreSQL integration.
"""
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                try:
                    ...
                    logger.info(f"Viewed {badges_count} badges")
                    response.success()
                except Exception as e:
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                try:
                    ...
                    logger.info(f"Viewed points: {points}")
                    response.success()
                except Exception as e:
                    logger.error(f"Error processing points: {str(e)}")
                    response.failure(f"Error processing points: {str(e)}")
            else:
                logger.error(f"View points failed: {response.status_code} - {response.text}")
                response.failure(f"View points failed: {response.status_code}")
    ...
    def view_analytics(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    logger.info("Admin viewed analytics")
                    response.success()
                except Exception as e:
                    logger.error(f"Error processing analytics: {str(e)}")
                    response.failure(f"Error processing analytics: {str(e)}")
            else:
                logger.error(f"Admin view analytics failed: {response.status_code} - {response.text}")
                response.failure(f"Admin view analytics failed: {response.status_code}")
